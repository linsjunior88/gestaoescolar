                                    ...
                                    const processarProfessores = async () => {
                                        for (const professor of data) {
                                            ...
                                            row.innerHTML = `
                                                ...
                                                <td>${turmasHTML}</td>
                                                <td class="text-center">
                                                    <button class="btn btn-sm btn-outline-primary editar-professor" data-id="${professor.id_professor}">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                    <button class="btn btn-sm btn-outline-danger excluir-professor" data-id="${professor.id_professor}">
                                                        <i class="fas fa-trash"></i>
                                                    </button>
                                                </td>
                                            `;
                                        ...
                                        document.querySelectorAll('.editar-professor').forEach(btn => {
                                            btn.addEventListener('click', function(event) {
                                                ...
                                                console.log("%c Botão de edição clicado para professor ID: " + idProfessor, "background: #2ecc71; color: white; padding: 3px;");
...
function initProfessores() {
    ...
    if (formProfessor) {
        formProfessor.addEventListener('submit', function(e) {
            ...
            feedbackElement.className = 'alert mt-3';
...
function initProfessores() {
    ...
    if (formProfessor) {
        formProfessor.addEventListener('submit', function(e) {
            ...
            feedbackElement.className = 'alert mt-3';
            feedbackElement.style.display = 'none';
            formProfessor.appendChild(feedbackElement);
            
            // Função para mostrar feedback
            function mostrarFeedback(mensagem, tipo) {
                feedbackElement.textContent = mensagem;
                feedbackElement.className = `alert mt-3 alert-${tipo}`;
            ...
            function processarRespostaAPI(response, mensagemErro) {
                if (!response.ok) {
                    if (response.status === 404) {
                        return { status: "error", message: "Recurso não encontrado" };
                    } else if (response.status === 400) {
                        return response.json().then(data => {
                            return { status: "error", message: data.detail || mensagemErro };
                        });
                    } else {
                        throw new Error(`${mensagemErro}: ${response.status} ${response.statusText}`);
                    }
                }
                return response.json();
            }
            ...
            // Desabilitar o botão de submit para evitar cliques duplos
            const submitButton = formProfessor.querySelector('button[type="submit"]');
            if (submitButton) {
                            ...
                            .then(linhas => {
                                ...
                                document.querySelectorAll('.editar-professor').forEach(btn => {
                                    btn.addEventListener('click', function(event) {
                                        ...
                                        const idProfessor = this.getAttribute('data-id');
                                        console.log("%c Botão de edição (localStorage) clicado para professor ID: " + idProfessor, "background: #e74c3c; color: white; padding: 3px;");
                                        editarProfessor(idProfessor);
                                    });
                                });
    ...
    function editarProfessor(idProfessor) {
        console.log("%c Editando professor ID: " + idProfessor, "background: #3498db; color: white; padding: 5px; font-size: 16px;");
        
        // Verificar se o ID é válido
        if (!idProfessor) {
            console.error("ID de professor inválido!");
            alert("ID de professor inválido para edição!");
            return;
        }
        
        // Buscar os dados do professor
        console.log("Buscando dados do professor na API:", CONFIG.getApiUrl(`/professores/${idProfessor}`));
