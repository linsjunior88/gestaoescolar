...
Esta versão utiliza uma abordagem alternativa para lidar com problemas de codificação.
"""
...
# Configuração da API
app = FastAPI(title="API de Acesso Direto ao Banco - Versão Corrigida")
...
# Alterar a codificação padrão para lidar com caracteres especiais
...
def get_db_connection():
    try:
        # Conectar usando a string de conexão direta em vez de parâmetros individuais
        conn_string = "dbname='gestao_escolar' user='postgres' password='4chrOn0s' host='localhost' port='5432'"
...
@app.get("/")
def read_root():
    return {"message": "API de Acesso Direto ao Banco - Versão Corrigida", "status": "online"}
...
if __name__ == "__main__":
    uvicorn.run("direct_db_access_fixed:app", host="127.0.0.1", port=4000, reload=True) 